...
std::string HttpResponse::to_string() const {
	...
	std::string new_phrase = default_status_reasons.find(status_code)->second;
    ...
    if (status_code != 200) {
	if (status_code == 401) {
	    ss << "WWW-Authenticate: Basic realm=\"myhttp-cs252\"";
	}
	ss << "\r\n";
	return ss.str();
    }
    ss << "Content-Length: " << message_body.length() << "\r\n";
    ss << "Content-Type: " << headers.find("Content-Type")->second << "\r\n";
    ss << "\r\n";
    ss << message_body;
    ss << "Hello CS252!\r\n";
    return ss.str();
